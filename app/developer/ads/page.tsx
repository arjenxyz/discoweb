      ...
      <div className="flex flex-col gap-4">
        <div className="rounded-2xl border border-white/10 bg-[#0b0d12] p-5">
          ...
          <div className="flex gap-2">
            <input
              ...
              onBlur={(e) => {
                if (e.target.value) void fetchInviteInfo(e.target.value);
              }}
            ...
            <button
              type="button"
              onClick={() => void fetchInviteInfo(inviteUrl)}
          ...
          {preview && (
            <div className="mt-3 rounded-xl border border-[#5865F2]/20 bg-[#5865F2]/5 p-3 flex flex-wrap items-center gap-3">
              {preview.server_icon ? (
                ...
                <div className="w-10 h-10 rounded-lg bg-black/40 flex items-center justify-center text-white/40 text-xs">—</div>
              )}
        ...
        <div className="flex flex-col gap-2 max-h-[600px] overflow-y-auto pr-1">
          {ads.map((ad) => (
            <div
              key={ad.id}
              className={`flex flex-wrap items-center gap-3 rounded-2xl border p-4 transition-all ${
                ad.active ? 'border-emerald-500/30 bg-emerald-500/5' : 'border-white/10 bg-[#0b0d12] opacity-80'
              }`}
            >
              {ad.server_icon ? (
