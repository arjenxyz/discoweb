...
export async function POST(request: Request, ctx: RouteCtx) {
  ...
  if (!botToken) return NextResponse.json({ error: 'missing_bot_token' }, { status: 500 });
  ...
  try {
    const botMax = await getBotMaxRolePosition(botToken, guildId);
    const created = await createGuildRole(botToken, guildId, {
      name: row.role_name,
    ...
    if (row.role_icon_url) {
      ...
      if (b64) {
        try {
          await setGuildRoleIcon(botToken, guildId, created.id, b64);
        } catch (e) {
    ...
    if (!canBotManageRole(botMax, position)) {
      await fetch(`https://discord.com/api/guilds/${guildId}/roles/${created.id}`, {
        method: 'DELETE',
    ...
    if (body.auto_assign) {
      await assignMemberRole(botToken, guildId, targetUserId, created.id);
    ...
    const { data: updated, error: updErr } = await supabase
      ...
      .update({
        ...
        source: body.raffle_label ? 'raffle_winner' : row.source,
