...
export async function redeemPromoCode(params: {
  ...
  if (!normalizedCode) {
    return fail('invalid_payload', 'Promosyon kodu boş olamaz.', 400);
  }
  ...
  if (!server) {
    return fail('server_not_found', 'Sunucu bulunamadı veya kurulum tamamlanmamış.', 404);
  }

  const { data: promo, error: promoError } = await supabase
    .from('promotions')
    .select(
      'id,code,value,status,expires_at,max_uses,used_count,per_user_limit,server_id,created_by,created_by_username,created_by_avatar_url',
    )
  ...
  if (!Number.isFinite(packageAmount) || packageAmount <= 0) {
    return fail('invalid_value', 'Promosyon değeri geçersiz.', 500);
  }
