...
-- Backfill from legacy per-server maintenance_flags:
-- prefer any active row; otherwise keep inactive.
DO $$
DECLARE
  r record;
BEGIN
...
-- Public/anon may read flags (status page / clients); writes only via service_role.
