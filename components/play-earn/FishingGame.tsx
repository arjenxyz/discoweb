...
export default function FishingGame({ session, onEnd, onCancel }: Props) {
  ...
  useEffect(() => {
    ...
    sprites.forEach((sprite) => {
      const img = new Image();
      img.src = `/games/fish/${sprite}`;
      imagesRef.current[sprite] = img;
    });
    const bg = new Image();
    bg.src = '/games/fish/background_terrain.png';
    bgRef.current = bg;
  }, [session.manifest]);
  ...
  const finishSession = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/member/play-earn/session/end', {
        method: 'POST',
  ...
  const handleClick = async (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    try {
      const res = await fetch('/api/member/play-earn/session/catch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
